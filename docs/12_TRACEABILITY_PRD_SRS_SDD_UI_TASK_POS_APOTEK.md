...
| Hierarchical Category System (Tree) | `PRD-CAT-002`, `FR-CAT-002`, `TASK-BE-003B`, `TASK-FE-003B`, `ADR-0001` | Backend self-referencing `Category`, Redis cache `categories:tree`, `categories:flat`, `CategoryTreeView` component | Category tree integration tests; Redis cache invalidation tests; depth 3 limit validation |
| Notification Center Dynamic Synthesis | `PRD-NOTIF-001`, `FR-NOTIF-001`, `TASK-BE-NOTIF-001`, `TASK-FE-NOTIF-001`, `ADR-0002` | `NotificationsService` parallel query synthesis (low stock, expiring batch), `NotificationCenter`, `NotificationBell`, LocalStorage read state | Notifications API synthesis tests; LocalStorage client-state read/delete UI tests |
